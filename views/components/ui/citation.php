...
$host = $url ? preg_replace('#^www\.#', '', (string) (parse_url((string) $url, PHP_URL_HOST) ?: $url)) : null;
$label = trim('Source '.$index.($title ? ': '.$title : ''));
$citationUrl = safe_url(is_string($url) ? $url : null);
?>
<span
    data-slot="citation"
    x-data="{
...
>
    <button
        type="button"
        :id="$id('hot-citation') + '-trigger'"
        aria-label="<?= e($label) ?>"
        :aria-expanded="open"
        :aria-controls="$id('hot-citation')"
        @click="open = !open"
        @focus="show()"
        @blur="hide()"
    ...
    >
        <?php if ($title): ?>
            <span class="block font-medium text-popover-foreground"><?= e($title) ?></span>
        <?php endif; ?>
        ...
        <?php if ($url): ?>
            ...
            ], '<span class="truncate">'.e($host).'</span><i data-lucide="external-link" class="size-3 shrink-0" aria-hidden="true"></i>') ?>
        <?php endif; ?>

        <?php if ($snippet): ?>
            <span class="mt-2 block text-xs text-muted-foreground"><?= e($snippet) ?></span>
        <?php endif; ?>
