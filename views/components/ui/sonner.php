...
$enterStartY = $isTop ? '-translate-y-2' : 'translate-y-2';
?>
<div
    ...
    x-data="{
        ...
        init() {
            ...
            if (window.__hotToasterActive) { this.disabled = true; return; }
            window.__hotToasterActive = true;
        },
        destroy() { if (! this.disabled) window.__hotToasterActive = false; },
        add(t) {
    ...
    >
        <template x-for="(t, idx) in toasts" :key="t.id">
            <div
                ...
                x-transition:enter-start="opacity-0 <?= $enterStartY ?>"
            ...
            >
                <template x-if="t.type === 'success'"><i data-lucide="circle-check" class="text-emerald-500 mt-0.5 size-4 shrink-0"></i></template>
                <template x-if="t.type === 'error'"><i data-lucide="circle-x" class="text-destructive mt-0.5 size-4 shrink-0"></i></template>
                <template x-if="t.type === 'warning'"><i data-lucide="triangle-alert" class="text-amber-500 mt-0.5 size-4 shrink-0"></i></template>
                <template x-if="t.type === 'info'"><i data-lucide="info" class="text-sky-500 mt-0.5 size-4 shrink-0"></i></template>
                <template x-if="t.type === 'loading'"><i data-lucide="loader-circle" class="text-muted-foreground mt-0.5 size-4 shrink-0 animate-spin"></i></template>
