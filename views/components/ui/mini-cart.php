...
$lineItems = [];
foreach (is_array($items) ? array_values($items) : [] as $i => $it) {
    $it = is_array($it) ? $it : [];
    $lineItems[] = [
        'id' => $i,
        'name' => (string) ($it['name'] ?? ''),
...
>
    <button
        ...
        :aria-expanded="open"
        :aria-controls="$id('hot-mini-cart')"
    ...
    >
        <i data-lucide="shopping-cart" class="size-5" aria-hidden="true"></i>
        <span
            x-show="count > 0"
            x-cloak
    ...
    >
        ...
        <div x-show="isEmpty" class="flex flex-col items-center justify-center gap-3 px-6 py-12 text-center">
            <span class="bg-muted text-muted-foreground flex size-12 items-center justify-center rounded-full">
                <i data-lucide="shopping-cart" class="size-6" aria-hidden="true"></i>
            </span>
            <div class="space-y-1">
        ...
        <div x-show="! isEmpty" class="shrink-0 border-t">
            ...
            <div class="flex flex-col gap-2 p-4">
                <?= $this->uiButton(['class' => 'w-full'], 'Checkout') ?>
                <?= $this->uiButton(['href' => '#', 'variant' => 'outline', 'class' => 'w-full'], 'View cart') ?>
            </div>
        </div>
    </div>
    </template>
