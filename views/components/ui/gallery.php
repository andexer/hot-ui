...
$images = array_values(array_map(static function ($img) {
    if (is_string($img)) {
...
>
    <div class="grid <?= e($gridCols) ?> gap-2">
        <?php foreach ($images as $i => $img): ?>
            <?php
            $viewLabel = $img['alt'] !== '' ? 'View image: '.$img['alt'] : 'View image '.($i + 1);

            $thumbSrc = safe_url(is_string($img['thumb']) ? $img['thumb'] : null);
            ?>
            <button
                type="button"
                @click="show(<?= (int) $i ?>)"
                aria-label="<?= e($viewLabel) ?>"
                class="group bg-muted focus-visible:ring-ring relative aspect-square overflow-hidden <?= e($rounded) ?> border outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
            >
                <img src="<?= e($thumbSrc) ?>" alt="<?= e($img['alt']) ?>" loading="lazy" class="size-full object-cover transition-transform duration-300 group-hover:scale-105" />
            </button>
        <?php endforeach; ?>
    </div>
        ...
        >
            ...
            >
                ...
                <div class="relative flex flex-1 items-center justify-center px-4 pb-6" @click.self="open = false">
                    ...
                    >
                        <i data-lucide="chevron-left" class="hot-rtl-flip size-6"></i>
                    </button>
                    ...
                    >
                        <i data-lucide="chevron-right" class="hot-rtl-flip size-6"></i>
                    </button>
                </div>
            </div>
        </div>
