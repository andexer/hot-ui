...
/**
 * Hot calendar — Alpine day-picker mirroring shadcn/react-day-picker's
 * <Calendar>. Supports: mode single|multiple|range, numberOfMonths,
 * captionLayout label|dropdown, showWeekNumber, disabled matchers (array |
 * {before, after, dayOfWeek}), range duration bounds (minDays/maxDays, plus
 * the legacy min/max aliases), out-of-range date bounds (minDate/maxDate with
 * 'disable'|'flag' policy), required, startMonth/endMonth, disableNavigation,
 * modifiers + modifier classNames, weekStart and the calendar:* control hooks.
 *
 * The date helpers below (ymd/parseDate/sameDay/addMonths) are exported for
 * the sibling islands (date-picker, datetime-picker).
 */

export type CalendarMode = 'single' | 'multiple' | 'range';

/** Either end of a range: 'Y-m-d' string or Date; both ends nullable. */
export interface RangeSeed {
    from?: string | Date | null;
    to?: string | Date | null;
}

/** Every shape `value` may take across the three modes. */
export type CalendarSelection = string | string[] | RangeSeed | null;

/** What the `value` getter hands back, per mode (null while nothing is selected). */
export type CalendarValue = string | string[] | { from: string | null; to: string | null } | null;

/** A grid day: a real Date stamped with prev/next-month status. */
export type CalendarDay = Date & { __outside?: boolean };

/** Matcher list accepted by the `disabled` prop: array | {before,after,dayOfWeek}. */
export type DisabledMatcher =
    | unknown[]
    | { before?: unknown; after?: unknown; dayOfWeek?: unknown }
    | null;

export interface CalendarConfig {
    mode?: CalendarMode;
    value?: CalendarSelection;
    /** Instance handle for targeted calendar:* events and stamped updates. */
    ...
    /** Range duration bounds in DAYS (legacy name; prefer minDays/maxDays). */
    min?: number | null;
    max?: number | null;
    /** Explicit range-duration bounds in days. */
    minDays?: number | null;
    maxDays?: number | null;
    /** Out-of-range date bounds (Y-m-d). */
    minDate?: string | Date | null;
    maxDate?: string | Date | null;
    /** 'disable' (prevent selection) | 'flag' (allow + show red). */
    outOfRange?: 'disable' | 'flag';
    ...
    modifiersClass?: Record<string, string>;
}
...
/** Where a day sits inside the current range (drives the data-range-* attrs). */
...
/** Source stamped on outgoing `calendar:updated` events. */
export type CalendarSource = 'select' | 'set' | 'set-range' | 'today' | 'clear' | 'value';

export interface HotCalendarController {
    calendarId: string | null;
    mode: CalendarMode;
    locale: string;
    todayLabel: string;
    ...
    hover: Date | null;
    focusedDate: Date;
    _hooks: Record<string, (e: Event) => void> | null;
    _rootEl: HTMLElement | null;

    init(): void;
    destroy(): void;
    get value(): CalendarValue;
    set value(v: CalendarSelection);
    /** Seed the selection and scroll it into view; true when anything changed. */
    setValue(v: CalendarSelection): boolean;
    reveal(d: Date): void;
    get months(): Date[];
    monthLabel(m: Date): string;
    weeksFor(m: Date): CalendarDay[][];
    weekNumber(week: CalendarDay[]): number;
    isOutside(d: Date, m: Date): boolean;
    isToday(d: Date): boolean;
    isOutOfRange(d: Date): boolean;
    isDisabled(d: Date): boolean;
    isSelected(d: Date): boolean;
    rangeIs(d: Date): RangeParts;
    modifierClass(d: Date): string;
    select(d: Date): void;
    notify(source: CalendarSource): void;
    get canPrev(): boolean;
    get canNext(): boolean;
    prev(): void;
    next(): void;
    isFocused(d: Date): boolean;
    dayLabel(d: Date): string;
    _viewContains(d: Date): boolean;
    _focus(d: Date): void;
    moveFocus(days: number): void;
    moveFocusMonths(n: number): void;
    focusWeekEdge(end: boolean): void;
    isRtl(): boolean;
    onDayKeydown(e: KeyboardEvent, d: Date): void;
    get years(): number[];
    get monthNames(): string[];
    setMonth(m: string | number): void;
    setYear(y: string | number): void;
...
interface CalendarScope {
    $root: HTMLElement;
    $dispatch(name: string, detail?: unknown): CustomEvent;
}

type Live = HotCalendarController & CalendarScope;
 ...
 */
export function createHotCalendar(config: CalendarConfig = {}): HotCalendarController {
    return {
        ...
        init() {
            ...
            if (this.mode === 'single') this.single = config.value ? parseDate(config.value) : null;
            ...
            const seeded =
                config.defaultMonth && typeof config.defaultMonth === 'string'
            ...
            const onToday = (e: Event): void => {
                if (this.mode !== 'single' || !mine(e)) return;
                if (this.setValue(ymd(new Date()))) this.notify('today');
            };
            const onSet = (e: Event): void => {
                if (this.mode !== 'single' || !mine(e)) return;
                const t = parseDate(payload(e));
                if (t && this.setValue(ymd(t))) this.notify('set');
            };
            // Detail: { from: 'YYYY-MM-DD'|Date|null, to: 'YYYY-MM-DD'|Date|null, id? }.
            const onSetRange = (e: Event): void => {
                if (this.mode !== 'range' || !mine(e)) return;
                const d = (((e as CustomEvent).detail ?? {}) as RangeSeed) || {};
            ...
            const onClear = (e: Event): void => {
                ...
                if (this.setValue(empty)) this.notify('clear');
            };
            this._hooks = {
            ...
            this._rootEl = (this as Live).$root;
            for (const target of [window, this._rootEl]) {
                for (const name in this._hooks) target.addEventListener(name, this._hooks[name] as (e: Event) => void);
            }
        },
        ...
        destroy() {
            if (!this._hooks || !this._rootEl) return;
            for (const target of [window, this._rootEl]) {
                for (const name in this._hooks) {
                    target.removeEventListener(name, this._hooks[name] as (e: Event) => void);
                }
            }
        },
        ...
        // …] · range { from, to }.
        get value(): CalendarValue {
            if (this.mode === 'single') return this.single ? ymd(this.single) : null;
            if (this.mode === 'multiple') return this.multiple.map(ymd);
        ...
        setValue(v: CalendarSelection): boolean {
            ...
            const seed = (v && typeof v === 'object' && !Array.isArray(v) ? v : {}) as RangeSeed;
            const from = seed.from ? parseDate(seed.from) : null;
            const to = seed.to ? parseDate(seed.to) : null;
            const key = (a: Date | null, b: Date | null): string => `${this.fmt(a)}/${this.fmt(b)}`;
        ...
        weeksFor(m: Date): CalendarDay[][] {
            ...
            for (let w = 0; w < 6; w++) {
                ...
                for (let d = 0; d < 7; d++) {
                    const day = new Date(start) as CalendarDay;
        ...
        isOutOfRange(d: Date): boolean {
            return Boolean(
                (this.minDate && d.getTime() < this.minDate.getTime())
        ...
        isDisabled(d: Date): boolean {
            ...
            if (!c) return false;
            if (Array.isArray(c)) return c.some((x) => sameDay(parseDate(x), d));
            if (typeof c === 'object') {
                if (c.before) {
                    const before = parseDate(c.before);
                    if (before && d.getTime() < before.getTime()) return true;
                }
                if (c.after) {
                    const after = parseDate(c.after);
                    if (after && d.getTime() > after.getTime()) return true;
                }
                if (Array.isArray(c.dayOfWeek) && c.dayOfWeek.includes(d.getDay())) return true;
            }

            return false;
        },
        isSelected(d: Date): boolean {
            if (this.mode === 'single') return sameDay(this.single, d);
            if (this.mode === 'multiple') return this.multiple.some((x) => sameDay(x, d));
        ...
        rangeIs(d: Date): RangeParts {
            ...
            if (!from) return { selected: false, start: false, end: false, middle: false };
        ...
        modifierClass(d: Date): string {
            ...
            for (const name in this.modifiers) {
                ...
                if (list?.some((x) => sameDay(parseDate(x), d))) cls += ` ${(this.modifiersClass[name] ?? '')}`;
            }
        ...
        notify(source: CalendarSource): void {
            ...
            (this as Live).$dispatch('calendar:updated', { id: this.calendarId, mode: this.mode, value, source });
            if (source === 'select') (this as Live).$dispatch('calendar-change', value);
        },
        ...
        _focus(d: Date): void {
            ...
            const key = ymd(d);
            const root = (this as Live).$root;
            setTimeout(() => {
                root.querySelector<HTMLElement>(`[data-day="${key}"]`)?.focus();
            }, 0);
        },
        moveFocus(days: number): void {
        ...
        isRtl(): boolean {
            ...
            return Boolean(
                el && el.nodeType === 1 && typeof getComputedStyle === 'function'
...
export default {
    ...
    register({ alpine }: HotContext): void {
        alpine.data('hotCalendar', createHotCalendar as never);
    },
