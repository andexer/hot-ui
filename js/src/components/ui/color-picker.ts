...
/**
 * hotColorPicker — controller for the color-picker.
 *
 * All state and logic extracted verbatim from the template's inline x-data:
 * hex normalization (#rgb/#rrggbb), HSL→hex for the hue slider drag, hex→hue
 * to position it, live validation of the text field and preset picks. The
 * colour itself flows through $hot.model.
 */
...
export interface ColorPickerConfig {
    ...
    disabled?: boolean;
    inline?: boolean;
    /** Initial text shown in the hex box (the raw starting colour). */
    input?: string | null;
    swatches?: string[];
}

export interface HotColorPickerController {
    open: boolean;
    disabled: boolean;
    inline: boolean;
    _model: HotModel;
...
export function createHotColorPicker(config: ColorPickerConfig): HotColorPickerController {
    return {
        open: false,
...
export default {
    ...
    register({ alpine }: HotContext): void {
        alpine.data('hotColorPicker', createHotColorPicker as never);
    },
