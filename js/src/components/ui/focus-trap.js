...
function releaseScrollLock() {
    if (scrollDepth > 0)
        scrollDepth -= 1;
    if (scrollDepth > 0 || !savedOverflow)
        return;
...
function releaseInert() {
    if (inertDepth > 0)
        inertDepth -= 1;
    if (inertDepth > 0 || INERTED.size === 0)
        return;
...
// Focusables
// ---------------------------------------------------------------------------
function collectFocusables(panel) {
...
// ---------------------------------------------------------------------------
function engage(panel, options) {
    const previous = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    if (options.lockScroll)
        acquireScrollLock();
    if (options.inert)
        acquireInert(layerRoot(panel));
    ...
    let frame = requestAnimationFrame(() => {
        frame = requestAnimationFrame(() => {
            if (cancelled || !panel.isConnected)
                return;
            const initial = panel.querySelector('[autofocus]') ??
    ...
    const onKeydown = (event) => {
        ...
        const index = current instanceof HTMLElement && current !== panel
 ...
 */
export function trapFocus(panel, active, options) {
    const current = ENGAGED.get(panel);
    if (active) {
        if (!current)
            ENGAGED.set(panel, engage(panel, options));
        return;
    }
    if (current) {
        current.release();
