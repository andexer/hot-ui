...
const FIELD_CONTROL = 'input:not([type=hidden]), textarea, select, [role="checkbox"], [role="switch"], [role="radiogroup"], [role="combobox"], [role="slider"], [role="spinbutton"]';
/**
 ...
 */
export const fieldDirective = (el, _directive, { cleanup }) => {
    let ourIds = [];
    ...
    const sync = () => {
        ...
        if (!control)
            return;
        const description = el.querySelector('[data-slot="field-description"]');
        const error = el.querySelector('[data-slot="field-error"]');
        const ids = [];
        if (description)
            ids.push(idFor(description, 'desc', 'field-desc'));
        if (error)
            ids.push(idFor(error, 'err', 'field-err'));
