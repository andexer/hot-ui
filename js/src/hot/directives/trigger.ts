 ...
 */
export const triggerDirective: DirectiveHandler = (el, { expression }, { evaluate, effect, cleanup }) => {
    const config = expression ? (evaluate(expression) as TriggerConfig) : {};
    ...
    const sync = (): void => {
        ...
        if (config.id && !control.id) control.id = config.id;
        if (config.haspopup) setAttr(control, 'aria-haspopup', config.haspopup === true ? 'true' : config.haspopup);
        if (config.controls) setAttr(control, 'aria-controls', config.controls);
        if (config.labelledby) setAttr(control, 'aria-labelledby', config.labelledby);
        if (config.describedby) setAttr(control, 'aria-describedby', config.describedby);
