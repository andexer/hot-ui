import type { ToastDetail, ToastFn } from './types.js';
...
function normaliseDetail(input?: string | ToastDetail): ToastDetail {
...
export function installToast(): void {
    ...
    const toast = ((input?: string | ToastDetail): void => {
        window.dispatchEvent(new CustomEvent<ToastDetail>('toast', { detail: normaliseDetail(input) }));
    }) as ToastFn;

    for (const kind of ['success', 'error', 'warning', 'info'] as const) {
        toast[kind] = (input?: string | ToastDetail): void => {
    ...
    toast.loading = (input?: string | ToastDetail): void => {
        window.dispatchEvent(new CustomEvent<ToastDetail>('toast', {
    ...
    toast.promise = <T>(promise: Promise<T> | (() => Promise<T>), messages: Parameters<ToastFn['promise']>[1] = {}): Promise<T> => {
        const id = `hot-tp-${String(++promiseCounter)}`;
        const text = (message: unknown, value?: unknown): string =>
            typeof message === 'function' ? String((message as (v: unknown) => unknown)(value)) : String(message ?? '');
        ...
        return Promise.resolve(typeof promise === 'function' ? promise() : promise)
            .then((data) => {
                window.dispatchEvent(new CustomEvent<ToastDetail>('toast-update', {
...
function readTokens(): Record<string, string> {
    ...
    if (out['--font-heading'] === out['--font-sans']) out['--font-heading'] = 'var(--font-sans)';

    return out;
}

export function installThemeExporter(): void {
    window.exportTheme = (): string => {
        ...
        const fmt = (tokens: Record<string, string>, keys: readonly string[]): string =>
            keys.filter((token) => tokens[token]).map((token) => `  ${token}: ${tokens[token]};`).join('\n');
