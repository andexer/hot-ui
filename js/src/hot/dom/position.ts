...
function baseCoords(reference: Rect, floating: Rect, placement: Placement, offset: number): { x: number; y: number } {
    const [rawSide, rawAlign] = placement.split('-') as [Side, Alignment | undefined];
    const align: Alignment = rawAlign ?? 'center';
    const side = rawSide;
    let x = reference.x;
    let y = reference.y;

    if (side === 'top') y -= floating.height + offset;
    if (side === 'bottom') y += reference.height + offset;
    if (side === 'left') x -= floating.width + offset;
    if (side === 'right') x += reference.width + offset;
...
function isOutsideViewport(viewport: Rect, floating: Rect, x: number, y: number, padding: number): boolean {
    return (
        x < padding ||
        y < padding ||
...
export function computePosition(
    ...
    const [rawSide] = config.placement.split('-') as [Side, Alignment | undefined];
    ...
    if (config.allowFlip && isOutsideViewport(viewport, floating, x, y, config.padding)) {
        side = opposite(side);
        const flipped: Placement = alignment ? `${side}-${alignment as Alignment}` : side;
    ...
    if (config.matchWidth && reference.width > 0) {
        ...
        if (floatingEl.style.minWidth !== w) floatingEl.style.minWidth = w;
    }
...
export interface AutoUpdater { stop(): void }
