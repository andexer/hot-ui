...
export function resolveControl(el: HTMLElement): HTMLElement {
    if (el.matches(FOCUSABLE_SELECTOR) && el.getAttribute('tabindex') !== '-1') return el;

    const nested = el.querySelector<HTMLElement>(
        'button:not([tabindex="-1"]), a[href]:not([tabindex="-1"]), input:not([type="hidden"]), select, textarea, [tabindex]:not([tabindex="-1"])',
    );

    return (nested ?? el.firstElementChild ?? el) as HTMLElement;
}

let hotIdCounter = 0;

export function mintId(prefix: string = 'hot'): string {
    hotIdCounter += 1;

    return `${prefix}-${String(hotIdCounter)}-${Math.random().toString(36).slice(2, 7)}`;
}

export function ensureId(node: HTMLElement, prefix: string = 'hot'): string {
    node.id ||= mintId(prefix);

    return node.id;
}

type IdMemo = Record<string, string>;

interface MemoOwner extends HTMLElement {
    _hotIds?: IdMemo;
}
 ...
 */
export function stableIds(owner: HTMLElement): (node: HTMLElement, role: string, prefix: string) => string {
    const el = owner as MemoOwner;
    const minted: IdMemo = (el._hotIds ??= {});
...
export function setAttr(node: Element, name: string, value: string | number | boolean | null | undefined): void {
 ...
 */
export function hotClosest(el: HTMLElement | null, selector: string): HTMLElement | null {
    let node: HTMLElement | null = el;
    while (node) {
        if (node.matches(selector)) return node;
        const back = (node as HTMLElement & { _x_teleportBack?: HTMLElement })._x_teleportBack;
